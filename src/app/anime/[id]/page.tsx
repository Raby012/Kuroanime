...
export default async function AnimePage({ params }: { params: { id: string } }) {
  ...
  const imdbLink = anime.externalLinks.find((l) => l.site === "IMDb" || l.url.includes("imdb.com"));
    ...
    <div className="min-h-screen">
      ...
      <div className="relative h-72 md:h-96 overflow-hidden">
        {anime.bannerImage ? (
          ...
          <Image src={anime.coverImage.extraLarge} alt={title} fill className="object-cover blur-sm scale-110" priority />
        )}
      ...
      <div className="max-w-7xl mx-auto px-4 -mt-40 relative z-10">
        <div className="flex gap-6 flex-col md:flex-row">
          ...
          <div className="flex-1 min-w-0 pt-4 md:pt-20">
            ...
            <div className="flex flex-wrap items-center gap-4 mb-4 text-sm text-gray-400">
              ...
              {anime.seasonYear && (
                <span className="flex items-center gap-1.5">
                  <Calendar size={15} className="text-brand" /> {anime.season} {anime.seasonYear}
                </span>
              )}
              {anime.popularity && (
                <span className="flex items-center gap-1.5">
                  <Users size={15} className="text-brand" /> {anime.popularity.toLocaleString()}
                </span>
              )}
              {anime.status && (
                <span className={`px-2 py-0.5 rounded text-xs font-semibold ${
                  anime.status === "RELEASING" ? "bg-green-900/50 text-green-400" :
                  anime.status === "FINISHED" ? "bg-surface-2 text-gray-400" :
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              {anime.genres.map((g) => (
                <span key={g} className="text-xs bg-surface-2 border border-white/10 text-gray-300 px-3 py-1 rounded-full">
                  {g}
                </span>
              ))}
            </div>
            ...
            {anime.studios.nodes.length > 0 && (
              <p className="text-sm text-gray-400 mb-4">
                Studio: <span className="text-brand font-medium">{anime.studios.nodes[0].name}</span>
              </p>
            )}
              ...
              {anime.characters.edges.map(({ node, role }) => (
                <div key={node.id} className="shrink-0 w-24 text-center">
                  <div className="w-24 h-24 rounded-full overflow-hidden mx-auto bg-surface-2">
                    <Image src={node.image.medium} alt={node.name.full} width={96} height={96} className="object-cover w-full h-full" />
                  </div>
              ...
              {related.map(({ node, relationType }) => (
                <div key={node.id} className="shrink-0">
                  <div className="relative">
                    <AnimeCard anime={{ id: node.id, title: node.title, coverImage: { large: node.coverImage.large, extraLarge: node.coverImage.large, medium: node.coverImage.large, color: null }, format: node.format } as any} size="sm" />
        ...
        {recs.length > 0 && (
          <section className="mt-12 mb-8">
            <h2 className="font-display text-2xl text-white mb-4">YOU MAY ALSO LIKE</h2>
            <div className="flex gap-4 overflow-x-auto pb-2">
              {recs.map(({ mediaRecommendation: rec }) =>
                rec ? (
                  <div key={rec.id} className="shrink-0">
                    <AnimeCard
                      anime={{ id: rec.id, title: rec.title, coverImage: { large: rec.coverImage.large, extraLarge: rec.coverImage.large, medium: rec.coverImage.large, color: null }, format: rec.format } as any}
                      size="sm"
                    />
                  </div>
                ) : null
