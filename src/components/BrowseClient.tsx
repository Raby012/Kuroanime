...
export default function BrowseClient() {
  ...
  const fetchAnimes = useCallback(async (reset: boolean, currentPage: number) => {
    ...
    try {
      ...
      // API returns the raw AniList response: { data: { Page: { media, pageInfo } } }
                ...
                {hasMore && !loading && (
                  <div className="flex justify-center mt-8">
                    <button onClick={() => fetchAnimes(false, page)}
                      className="bg-surface-1 hover:bg-surface-2 border border-white/8 text-white px-6 py-2.5 rounded-xl text-sm font-medium transition-all">
                      Load More
                    </button>
                  </div>
                )}
